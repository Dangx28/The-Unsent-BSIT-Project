...
export const searchSong = async (token: string, query: string) => {
  try {
    const response = await fetch(
      `https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&limit=10&type=track`,
      {
    ...
    alert(
      "Search failed due to browser blocking Spotify queries. USE a different browser. You can still submit your message but it will have no song.",
    );
    console.error(error);
    return [];
  }
